...
() => {
    ...
    for (var i = 0; i < rawItems.length; i++) {
        ...
        // Location: can be array [{name},...] or object {name} or nested location_tree
        var cluster = '';
...
async def enrich_bayut_dates(context, listings, raw_data=None):
    ...
    Uses parallel fetch (5 concurrent) for speed. Only enriches listings
    that don't already have a date. Checks existing raw_data for cached dates.
    """
    # First, try to copy dates from existing raw_data (avoids re-fetching)
    if raw_data:
        existing_dates = {}
    ...
    all_dates = {}
    batch_size = 30  # Larger batches since fetches are parallel now
